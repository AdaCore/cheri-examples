...
{
    printf("%s %zu %zu\n", __func__, sizeof(void *), sizeof(intptr_t));
    ...
    // causes an exception.
    get_heap((void *)got[0], allocation);
    int index = 1;
    /*
    do {
        printf(" %i @ %p: ", index, got + sizeof(intptr_t) * index);
